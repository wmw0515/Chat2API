...
export class PerplexityAdapter {
  ...
  async chatCompletion(request: ChatCompletionRequest): Promise<{ stream: Readable; sessionId: string }> {
    ...
    try {
      const response = await this.sendRequest('POST', QUERY_ENDPOINT, headers, data, 'stream')
      ...
      if (statusCode >= 400) {
        throw new Error(`HTTP ${statusCode}: upstream request failed`)
      }

      return { stream: response.data as Readable, sessionId: requestId }
    } catch (error) {
      const err = error instanceof Error ? error : new Error(String(error))
      throw new Error(this.formatNetworkError(err))
    }
  }
  ...
  async deleteSession(sessionId: string): Promise<boolean> {
    ...
    try {
      ...
      const response = await this.sendRequest('DELETE', deleteUrl, headers, requestBody)
      sessionCache.delete(cacheKey)
  ...
  async deleteAllChats(): Promise<boolean> {
    ...
    try {
      const response = await this.sendRequest('DELETE', deleteUrl, headers, { delete_all: true })
